...
#ifndef S_UTILS_VARIANTREADER_H
#define S_UTILS_VARIANTREADER_H
...
namespace sutils {
    ...
    private:
        ...
        Reader *_get_reader(ReadMode m) const {
            ...
            auto m_i = std::size_t(m);

            if(m_i >= std::size_t(ReadMode::END)) return nullptr;
        ...
        Reader *_readers[std::size_t(ReadMode::END)];

        bool _delete_reader_after[std::size_t(ReadMode::END)];
    };
