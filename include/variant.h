...
#ifndef S_UTILS_VARIANT_H
#define S_UTILS_VARIANT_H
...
namespace sutils {
     ...
     * bool
     * Variant::Integer - Same as long long
     * Variant::Real - Same as double
    ...
    public:
        ...
        Variant(const long long& i) : _var(Integer(i)) {}
        Variant(const std::size_t& i) : _var(Integer(i)) {}
        ...
        Variant& operator=(const long long& i) { _var = Integer(i); return *this; }
        Variant& operator=(const std::size_t& i) { _var = Integer(i); return *this; }
         ...
         */
        std::size_t typeId() const { return _var.index(); }
        ...
        template<typename T>
        constexpr static std::size_t getTypeId() { return _get_type_id_aux<T>(); }

        constexpr static const char *typeName(std::size_t id);
    ...
    private:
        ...
        template<typename T, typename U = std::variant_alternative_t<0, InternalVariant>, std::size_t N = 0>
        struct _get_type_id {

            static const std::size_t value = _get_type_id<T, std::variant_alternative_t<N+1, InternalVariant>, N+1>::value;
        };
        ...
        struct _get_type_id <T, U, std::variant_size<InternalVariant>::value> {
            ...
            static const std::size_t value = std::variant_npos;
        };

        template<typename T>
        constexpr static std::size_t _get_type_id_aux() { return _get_type_id<T>::value; }

        InternalVariant _var;
    };

    constexpr const char *Variant::typeName(std::size_t id) {
